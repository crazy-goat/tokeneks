...
- [ ] **D1** — Repeated SQLite connection boilerplate in 5 functions → [details](01-duplication.md#d1--repeated-sqlite-connection-boilerplate)
- [ ] **D2** — Nearly identical SQL in the two branches of `ocSessions` → [details](01-duplication.md#d2--nearly-identical-sql-queries-in-ocsessions)
...
- [ ] **D7** — "Dominant model" loop written 3× → [details](01-duplication.md#d7--dominant-model-computation-duplicated-3)
- [ ] **D8** — Cost-per-1M computation duplicated in every list/total → [details](01-duplication.md#d8--cost-per-1m-tokens-computation-duplicated)
- [ ] **D9** — `byModel` aggregation + per-model summarize duplicated 4× → [details](01-duplication.md#d9--bymodel-aggregation--per-model-summarize-duplicated)
...
- [ ] **D14** — `parseTS` closure defined twice → [details](01-duplication.md#d14--duplicated-parsets-closure)
...
- [ ] **A6** — `isClaudeModel` hardcodes IDs while prices are file-configurable → [details](02-architecture.md#a6--isclaudemodel-hardcodes-model-ids-while-prices-are-file-configurable)
...
- [ ] **R1** — New `*sql.DB` pool opened/closed on every call → [details](04-resources-and-leaks.md#r1--a-new-sqldb-connection-pool-is-opened-and-closed-on-every-call)
...
- [ ] **C1** — Divide-by-zero → `NaN`/`+Inf` in pct/per-1M → [details](05-correctness-bugs.md#c1--division-by-zero--naninf-in-percentage-and-per-1m-calculations)
...
- [ ] **C4** — Silently swallowed errors hide data/failures → [details](05-correctness-bugs.md#c4--ignored-errors-swallow-data-and-hide-failures)
...
- [ ] **S1** — Dead code: `repeatByte`, `piStepActualCost` → [details](06-code-smells.md#s1--dead-code)
...
- [ ] **S7** — gofmt violation: mis-indented brace (`pi_agent.go:147`) → [details](06-code-smells.md#s7--gofmt-violation-mis-indented-brace)
...
- [ ] **S12** — Inconsistent resource closing (manual `Close` vs `defer`) → [details](06-code-smells.md#s12--inconsistent-closing-of-resources-within-one-file)
</content>
